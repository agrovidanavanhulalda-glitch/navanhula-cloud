...
BEGIN
  ...
  -- Caixa aberto obrigatório para numerário
  ...
  -- Validação de stock (apenas produtos com product_id)
  ...
  -- Validação de voucher (sem resgatar ainda)
  ...
  -- Persistir venda (triggers cuidam de financeiro/contabilidade/comissão)
  INSERT INTO sales (
    ...
    p_store_id, v_user_id,
    CASE WHEN v_payment='cash' THEN p_cash_register_id ELSE p_cash_register_id END,
  ...
  -- Persistir itens (trigger tr_handle_sale_item_stock baixa o stock)
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
  ...
  -- Resgate do voucher DEPOIS da venda persistida
  ...
  -- Movimento de caixa (apenas numerário)
...
REVOKE ALL ON FUNCTION public.pos_complete_sale(uuid,uuid,text,jsonb,numeric,numeric,numeric,numeric,text,text,text,text,text,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.pos_complete_sale(uuid,uuid,text,jsonb,numeric,numeric,numeric,numeric,text,text,text,text,text,text) TO authenticated;
