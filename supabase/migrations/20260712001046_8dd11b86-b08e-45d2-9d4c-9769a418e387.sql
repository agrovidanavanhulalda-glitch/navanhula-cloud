...
CREATE UNIQUE INDEX IF NOT EXISTS uq_sales_client_sale_id
  ON public.sales(company_id, client_sale_id)
  WHERE client_sale_id IS NOT NULL;

-- Replace pos_complete_sale with idempotent + wallet-crediting version
...
BEGIN
  ...
  IF p_client_sale_id IS NOT NULL THEN
    SELECT id, total, profit
      INTO v_sale_id, v_existing_total, v_existing_profit
    FROM sales
    WHERE company_id = v_company_id AND client_sale_id = p_client_sale_id
    LIMIT 1;
  ...
  INSERT INTO sales (
    ...
    seller_name, notes, cost_total, profit, synced, client_sale_id
  ) VALUES (
    ...
    p_seller_name, p_notes, v_cost_total, v_profit, true, p_client_sale_id
  ...
  -- Wallet credit for non-cash, non-voucher payments (moved from client-side syncQueue)
  IF v_payment <> 'cash' AND v_voucher_id IS NULL THEN
    PERFORM public.credit_wallet_from_sale(
      p_store_id, p_payment_method, v_total, v_sale_id
    );
  END IF;
